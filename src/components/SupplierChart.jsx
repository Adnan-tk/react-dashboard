...
export default function SupplierChart() {
  ...
  const fetchData = () => {
    const params = { ...debouncedFilters, group_by: 'Supplier' }
    api.get('/api/breakdown/', { params })
      .then((res) => {
        const arr = Array.isArray(res.data) ? res.data : []
        const labels = arr.map((r) => r['Supplier'] ?? '')
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'SupplierChart')
    XLSX.writeFile(wb, 'supplierchart.xlsx')
  }

  return (
    <div ref={chartRef} className="bg-white dark:bg-gray-800 p-4 shadow rounded">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-xl font-semibold">📊 SupplierChart</h2>
