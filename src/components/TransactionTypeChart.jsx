...
export default function TransactionTypeChart() {
  ...
  const fetchData = () => {
    const params = { ...debouncedFilters, group_by: 'TransactionType' }
    api.get('/api/breakdown/', { params })
      .then((res) => {
        const arr = Array.isArray(res.data) ? res.data : []
        const labels = arr.map((r) => r['TransactionType'] ?? '')
  ...
  const exportPDF = () => {
    html2canvas(chartRef.current).then(canvas => {
      ...
      pdf.save('transactiontypechart.pdf')
    })
  }
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'TransactionTypeChart')
    XLSX.writeFile(wb, 'transactiontypechart.xlsx')
  }

  return (
    <div ref={chartRef} className="bg-white dark:bg-gray-800 p-4 shadow rounded">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-xl font-semibold">📊 TransactionTypeChart</h2>
        <div className="flex items-center gap-2">
          ...
          <InsightBox chart="transactiontype" />
        </div>
      </div>
      <Bar data={data} />
    </div>
