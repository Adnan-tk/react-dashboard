...
export default function RegionBreakdownChart() {
  ...
  const fetchData = () => {
    const params = { ...debouncedFilters, group_by: 'Region' }
    api.get('/api/breakdown/', { params })
      .then((res) => {
        const arr = Array.isArray(res.data) ? res.data : []
        const labels = arr.map((r) => r['Region'] ?? '')
  ...
  const exportPDF = () => {
    html2canvas(chartRef.current).then(canvas => {
      ...
      pdf.save('regionbreakdownchart.pdf')
    })
  }
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'RegionBreakdownChart')
    XLSX.writeFile(wb, 'regionbreakdownchart.xlsx')
  }

  return (
    <div ref={chartRef} className="bg-white dark:bg-gray-800 p-4 shadow rounded">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-xl font-semibold">📊 RegionBreakdownChart</h2>
        <div className="flex items-center gap-2">
          ...
          <InsightBox chart="regionbreakdown" />
        </div>
      </div>
      <Bar data={data} />
    </div>
