...
export default function SalespersonTargetChart() {
  ...
  useEffect(() => {
    ...
    api.get('/api/salesperson-targets/', { params })
      .then((res) => {
        setChartData({
  ...
  const handleExportPDF = () => {
    html2canvas(chartRef.current).then(canvas => {
      ...
      pdf.save('SalespersonTargetChart.pdf')
    })
  }

  const handleExportExcel = () => {
    if (!chartData) return
    const rows = [['Salesperson', ...chartData.datasets.map(d => d.label)]]
    ...
    XLSX.writeFile(workbook, "SalespersonTargetChart.xlsx")
  }
