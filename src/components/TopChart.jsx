...
export default function TopChart() {
  ...
  const handleClick = (elems) => {
    if (elems.length > 0) setSelected(data.labels[elems[0].index])
  }
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'TopChart')
    XLSX.writeFile(wb, 'topchart.xlsx')
  }
  return (
    <div ref={chartRef} className="bg-white dark:bg-gray-800 p-4 shadow rounded">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-xl font-semibold">📊 TopChart</h2>
        <div className="flex items-center gap-2">
          ...
          <InsightBox chart="top" />
        </div>
      </div>
      <Bar data={data} options={{ onClick: (e, elements) => handleClick(elements) }} />
      {selected && (
        <Modal title={"Drilldown: " + selected} onClose={() => setSelected(null)}>
          <p>Show full analytics for <strong>{selected}</strong> (e.g., trend, region breakdown, etc.)</p>
        </Modal>
      )}
    </div>
  )
