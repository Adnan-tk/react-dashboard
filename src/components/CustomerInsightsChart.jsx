...
const metricOptions = ['Revenue', 'Profit']
...
export default function CustomerInsightsChart() {
  ...
  useEffect(() => {
    ...
    api.get('/api/customer-insights/', { params })
      .then((res) => {
        const m = res.data?.metric ?? metric
        setChartData({
          labels: res.data?.labels ?? [],
          datasets: [{
            label: `${m} (Top 10 Customers)`,
            data: res.data?.data ?? [],
            backgroundColor: m === 'Revenue' ? '#3b82f6' : '#22c55e'
          }]
        })
      })
  ...
  const handleExportPDF = () => {
    html2canvas(chartRef.current).then(canvas => {
      ...
      pdf.save(`CustomerInsights-${metric}.pdf`)
    })
  }

  const handleExportExcel = () => {
    if (!chartData) return
    const rows = [['Customer', metric]]
    ...
    XLSX.writeFile(wb, `CustomerInsights-${metric}.xlsx`)
  }
