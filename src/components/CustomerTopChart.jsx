...
export default function CustomerTopChart() {
  ...
  const fetchData = () => {
    ...
    api.get('/api/customer-top/', { params })
      .then((res) => {
        const arr = Array.isArray(res.data) ? res.data : []
        const labels = arr.map((r) => r['Customer'] ?? '')
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'CustomerTopChart')
    XLSX.writeFile(wb, 'customertopchart.xlsx')
  }

  return (
    <div ref={chartRef} className="bg-white dark:bg-gray-800 p-4 shadow rounded">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-xl font-semibold">📊 Top Customers by Revenue</h2>
