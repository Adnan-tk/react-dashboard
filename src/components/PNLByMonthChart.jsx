...
export default function PNLByMonthChart() {
  const { debouncedFilters } = useContext(FilterContext)
  const [chartData, setChartData] = useState(null)
  const chartRef = useRef(null)

  useEffect(() => {
    api.get('/api/pnl-by-month/', { params: debouncedFilters })
      .then(res => {
        setChartData({
      ...
      .catch(err => console.error('Error loading P&L by month', err))
  }, [debouncedFilters])
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(wb, 'PNLByMonth.xlsx')
  }
