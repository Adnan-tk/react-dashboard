...
export default function BreakdownChart() {
  ...
  const fetchData = () => {
    const params = { ...debouncedFilters, group_by: 'Store' }
    api.get('/api/breakdown/', { params })
      .then((res) => {
        const arr = Array.isArray(res.data) ? res.data : []
        const labels = arr.map((r) => r['Store'] ?? '')
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'BreakdownChart')
    XLSX.writeFile(wb, 'breakdownchart.xlsx')
  }

  return (
    <div ref={chartRef} className="bg-white dark:bg-gray-800 p-4 shadow rounded">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-xl font-semibold">📊 BreakdownChart</h2>
