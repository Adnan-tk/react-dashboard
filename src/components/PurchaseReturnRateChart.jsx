...
export default function PurchaseReturnRateChart() {
  ...
  useEffect(() => {
    ...
    api.get('/api/purchase-return-rate/', { params })
      .then(res => {
        setChartData({
          labels: res.data.labels,
          datasets: res.data.datasets.map((dataset, idx) => ({
            ...dataset,
            backgroundColor: idx === 0 ? '#3b82f6' : '#ef4444', // blue vs red
            barThickness: 20
          }))
        })
      })
      .catch(err => console.error('Error loading purchase return rate', err))
  }, [debouncedFilters])
  ...
  const handleExportPDF = () => {
    html2canvas(chartRef.current).then(canvas => {
      ...
      pdf.save('PurchaseReturnRate.pdf')
    })
  }
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(wb, 'PurchaseReturnRate.xlsx')
  }
    ...
    <div className="bg-white p-4 rounded shadow dark:bg-gray-800" ref={chartRef}>
      <div className="flex justify-between items-center mb-3">
        <h2 className="text-lg font-bold text-gray-800 dark:text-white">
          📦 Purchase Return Rate (Purchases vs Returns)
        </h2>
      </div>
